...
# 2. Özellik Seçimi (model_sonuclari'ndaki FAISS/k-NN yöntemiyle aynı şartlarda yarışmaları için aynı sütunları veriyoruz)
ozellik_kolonlari = [
...
# 6. Üretimde Çalışan FAISS/k-NN Sonuçlarıyla Karşılaştırma
df_uretim = pd.read_sql("SELECT Anomali_Durumu FROM model_sonuclari", baglanti)
uretim_anomali_sayisi = df_uretim['Anomali_Durumu'].sum()
lof_anomali_sayisi = df['LOF_Anomali_Durumu'].sum()

print("\n--- 🤖 YAPAY ZEKA ALGORİTMALARI KAPIŞMASI 🤖 ---")
print(f"Toplam İncelenen Log: {len(df)}")
print(f"🧭 Üretimdeki FAISS/k-NN Modelinin Bulduğu Anomali: {uretim_anomali_sayisi}")
...
ortak_tehditler = (df_uretim['Anomali_Durumu'] == 1) & (df['LOF_Anomali_Durumu'] == 1)
